<html>
	...
	<body background="eikones/cinema2.jpg">
		...
		<h1>  Οι αγαπημένες μου ταινίες </h1>
		<?php
		...
		<?php	
			...
			//Get all records of the movies from the favoritemovies  table for the user with the user's id.
			$sql = "SELECT movieid FROM favoritelist WHERE seriesid=0 AND userid='$userid' ";
			...
			//Output data of each row.
				while($row = $result->fetch_assoc()) {
					...
					<td><form action="playmovie.php?msid=<?php echo $value  ?>" method="POST">
						<input type="image" src="eikones/play.png" name="$value" alt="Submit" width="68" height="48">
					</form></td>
					
					<td><form action="removefromfavelist.php?id=<?php echo $userid ?>&msid=<?php echo $row2["id"]  ?>" method="POST">
