	...
	?>	
		...
		<?php	
			...
			while($row = $result->fetch_assoc()) {
				...
				<td>
					<form action="ratemovies.php?id=<?php echo $userid ?>&msid=<?php echo $row["id"] ?>" method="POST">		
				...
				<td><form  action="addcomments.php?id=<?php echo $userid ?>&msid=<?php echo $row["id"] ?>" method="POST">
				<p>
					<textarea cols="10"  rows="15" <input type="text"   style="width:400px; height:160px;"  name="comments"  id="commentsid" value="" /> 
				...
				<td><form action="playmovie.php?msid=<?php echo $row["id"]  ?>" method="POST">
				<input type="image" src="eikones/play.png" name="<?php echo $row["id"];?>" alt="Submit" width="68" height="48">
				</form></td>
				<td><form action="addtofavelist.php?id=<?php echo $userid ?>&msid=<?php echo $row["id"]  ?>" method="POST">
				<input type="image" src="eikones/add.png" name="<?php echo $row["id"];?>" alt="Submit" width="48" height="48">
				</form></td>
				<td><form action="addmovietowatchlist.php?id=<?php echo $userid ?>&msid=<?php echo $row["id"]  ?>" method="POST">
				<input type="image" src="eikones/add.png" name="<?php echo $row["id"];?>" alt="Submit" width="48" height="48">
				</form></td>
				<?php
				echo "</tr>";
