<html>
	...
	<body background="eikones/cinema2.jpg">
		<h1>  Οι αγαπημένες μου σειρές </h1>
		<?php
		...
		<?php	
			//Get all records of the series from the favoriteseries  table for the user with the user's id.
			$sql = "SELECT seriesid FROM favoritelist WHERE movieid=0 AND userid='$userid'  ";
			...
			if ($result->num_rows > 0) {
				...
				while($row = $result->fetch_assoc()) {
					...
					<td><form action="removefromfavelistseries.php?id=<?php echo $userid ?>&msid=<?php echo $row2["id"]  ?>" method="POST">
